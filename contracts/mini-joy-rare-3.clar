;; title: mini-joy-rare-3
;; summary: miniJoy Tier-3 NFT contract.

(impl-trait .sip009-nft-trait2.sip009-nft-trait)

(define-non-fungible-token mini-joy-rare-3-token uint)

(define-constant CONTRACT-OWNER tx-sender)
(define-constant MAX-SUPPLY u1)
(define-constant MINT-FEE u500)
(define-constant TOKEN-URI "ipfs://mini-joy/rare-3.json")
...
(define-public (mint)
  (let ((next-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? mini-joy-rare-3-token next-id tx-sender))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (let ((owner (unwrap! (nft-get-owner? mini-joy-rare-3-token token-id) ERR-TOKEN-NOT-FOUND)))
    (asserts! (is-eq tx-sender sender) ERR-NOT-AUTHORIZED)
    (asserts! (is-eq owner sender) ERR-NOT-TOKEN-OWNER)
    (asserts! (is-standard recipient) ERR-INVALID-RECIPIENT)
    (try! (nft-transfer? mini-joy-rare-3-token token-id sender recipient))
    (ok true)
  )
)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mini-joy-rare-3-token token-id))
)
